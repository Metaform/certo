...
/**
 * Generates a minimal, self-contained single-page PDF document so the seeder can serve a real
 * {@code application/pdf} body for the sample certificate binaries. Used only by
 * {@link ProviderCertificateSeeder} (active when {@code certo.seed-sample-data=true}); not a general-purpose
 * PDF library — it emits just enough valid PDF structure (catalog, pages, one page, Helvetica font, a text
 * stream with correct cross-reference offsets) to render a few lines of text.
 */
final class PdfGenerator {
     ...
     */
    static byte[] generate(String title, List<String> lines) {
