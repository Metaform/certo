 ...
 *   <li>{@code POST /companycertificate/available} — acknowledged only; retrieving the content required
 *       the legacy per-asset EDC pull, which is out of scope (old providers should use {@code /push}).</li>
 * </ul>
 */
@RestController
public class Ccm240ConsumerController {
    ...
    public ResponseEntity<Ccm240PushAck> push(@RequestBody Ccm240CertificatePush message) {
        ...
        LOG.info("Received v2.4.0 push -> certificate {} (exchange {})", certificateId, exchangeId);
