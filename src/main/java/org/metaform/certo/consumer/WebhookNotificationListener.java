...
/**
 * An out-of-process extension point: when {@code certo.consumer.notification-callback-url}
 * is configured, each recorded inbound event is POSTed as JSON to that URL, fire-and-forget. A client
 * listening there decides what to do and drives the consumer management API (retrieve/accept). Delivery is best-effort.
 * A failure is logged, never propagated as the event is already recorded and reconcilable.
 */
@Component
