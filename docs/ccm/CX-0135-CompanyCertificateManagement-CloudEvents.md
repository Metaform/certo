...
certificate, poll the fulfillment status of a request, query for certificates, get a certificate, and post acceptance
status updates.
...
Authorization **MUST** adhere to [CX-0000 Section 4](./CX-0000-CloudEventsFoundation.md#4-https-binding).
...
properties defined in [Section 4.4.4](#444-certificate-acceptance-events).
...
corresponding acceptance event in [Section 4.4.4](#444-certificate-acceptance-events).
...
[Section 4.4.4](#444-certificate-acceptance-events):
...
[certificate-consumer-notification-api.yaml](./certificate-consumer-notification-api.yaml).
...
Consumer's acceptance outcome is reported separately via [Section 4.4.4](#444-certificate-acceptance-events).
...
The Certificate Provider **MUST** expose the following endpoint to retrieve certificate data.
...
and `application/pdf`. The Client Consumer **SHOULD** set the `Accept` header to `multipart/related`.
...
#### 4.4.4 Certificate Acceptance Events
...
|------------------|----------------------------|
| **HTTP Method**: | POST                       |
| **URL Path**     | `POST /certificates/query` |
| **Content Type** | `application/json`         |
...
`POST /certificates/query` endpoint of the Certificate Provider. The content and structure of the URL (including any
...
HTTP/1.1 200 OK
Content-Type: application/json
Link: <https://provider.example.com/certificates/query?cursor=eyJvZmZzZXQiOjUwfQ>; rel="next",
      <https://provider.example.com/certificates/query?cursor=eyJvZmZzZXQiOjB9>; rel="prev"
...
#### 4.4.6 OpenAPI Specification

The Certificate Provider API is formally defined by the OpenAPI document
[certificate-provider-api.yaml](./certificate-provider-api.yaml).

#### 4.4.7 DSP Dataset Representation
...
[CX-0000-CloudEventsFoundation.md](./CX-0000-CloudEventsFoundation.md).
